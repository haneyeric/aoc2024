...
function checkPuzzle(puzzle, r, c) {
  let visitedDirection = [];
  for (let i = 0; i < rows; i++) {
    visitedDirection[i] = [];
    for (let j = 0; j < cols; j++) {
      visitedDirection[i][j] = [];
      for (let k = 0; k < 4; k++) {
        visitedDirection[i][j][k] = false;
      }
    }
  }
  ...
  do {
    if (
      ...
      switch (
        ...
        case "^":
          if (
            visitedDirection[position[0] + directions[direction][0]][
              position[1] + directions[direction][1]
            ][direction]
          ) {
            loops++;
